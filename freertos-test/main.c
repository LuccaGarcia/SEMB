 ...
 */
static volatile uint64_t ulCountOfTimerCallbackExecutions = 0;
static volatile uint64_t ulCountOfItemsSentOnQueue = 0;
static volatile uint64_t ulCountOfItemsReceivedOnQueue = 0;
static volatile uint64_t ulCountOfReceivedSemaphores = 0;
static volatile uint64_t ulCountOfSDKMutexEnters = 0;
static volatile uint64_t ulCountOfSDKSemaphoreAcquires = 0;
...
static void prvNonRTOSWorker() {
  ...
  while (true) {
    ...
    while (!time_reached(end_time)) {
      counter++;
      printf("Core %d: Busy work with mutex %llu\n", get_core_num(), counter);
      busy_wait_us(137384);
    }
...
int main(void) {
  ...
  /* Create the semaphore used by the FreeRTOS tick hook function and the event
   * semaphore task.
   *
...
static void vExampleTimerCallback(TimerHandle_t xTimer) {
  /* Argument xTimer is not used due to this callback fucntion is not reused and
   * use one timer only. */
  (void)xTimer;
...
static void prvQueueSendTask(void *pvParameters) {
  ...
  for (;;) {
    ...
    printf("Core %d - Thread '%s': Queue send %llu\n", get_core_num(),
...
static void prvQueueReceiveTask(void *pvParameters) {
  ...
  for (;;) {
    ...
    if (ulReceivedValue == 100UL) {
      ...
      printf("Core %d - Thread '%s': Queue receive %llu\n", get_core_num(),
             pcTaskGetName(xTaskGetCurrentTaskHandle()),
             ulCountOfItemsReceivedOnQueue);
    }
  }
...
static void prvEventSemaphoreTask(void *pvParameters) {
  ...
  for (;;) {
     ...
     * NOTE: A semaphore is used for example purposes.
     * In a real application it might be preferable to use a direct to task
     * notification, which will be faster and use less RAM. */
    xSemaphoreTake(xEventSemaphore, portMAX_DELAY);

    /* Count the number of times the semaphore is received. */
    ulCountOfReceivedSemaphores++;
    printf("Core %d - Thread '%s': Semaphore taken %llu\n", get_core_num(),
           pcTaskGetName(xTaskGetCurrentTaskHandle()),
           ulCountOfReceivedSemaphores);
  }
}
...
static void prvSDKMutexUseTask(void *pvParameters) {
  ...
  for (;;) {
    ...
    printf(
        "Core %d - Thread '%s': SDK Mutex Entered, sleeping for a while %llu\n",
        get_core_num(), pcTaskGetName(xTaskGetCurrentTaskHandle()),
        ulCountOfSDKMutexEnters);
    vTaskDelay(3000);
    printf("Core %d - Thread '%s': Sleep finished; SDK Mutex releasing %llu\n",
           get_core_num(), pcTaskGetName(xTaskGetCurrentTaskHandle()),
           ulCountOfSDKMutexEnters);
    mutex_exit(&xSDKMutex);
  }
...
static void prvSDKSemaphoreUseTask(void *pvParameters) {
  ...
  for (;;) {
    ...
    if (sem_acquire_timeout_us(&xSDKSemaphore, 250500)) {
      ...
      printf("Core %d - Thread '%sk: SDK Sem acquired %llu\n", get_core_num(),
             pcTaskGetName(xTaskGetCurrentTaskHandle()),
             ulCountOfSDKMutexEnters);
    } else {
